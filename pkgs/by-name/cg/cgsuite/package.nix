...
let
  ...
  coreLib = maven.buildMavenPackage {
    ...
    # scala-maven-plugin caches the compiled zinc bridge under the JVM's user.home,
    # which is not writable when Nix builds outside of the sandbox
    mvnParameters = "-f lib/core/pom.xml -Duser.home=$TMPDIR";
